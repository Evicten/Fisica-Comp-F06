...
class FCmatrix{
 ...
 public:
  ...
  FCmatrix(double**, int, int, string s = "matrix"); // matrixi, rows, columns
  FCmatrix(double*, int, int, string s = "matrix"); // matrix num vetor, rows, columns  
  ...
  virtual int GetRowIdx(int) const = 0;
  virtual void IdxPrint() = 0;
  virtual void Setvector(vector<Vec>&) = 0;
  virtual vector<Vec> GetMat() = 0;
  // friend methods
